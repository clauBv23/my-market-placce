...
import KriptoBirdz from '../abis/KriptoBirdz.json'
...
class App extends Component {
    ...
    async loadWeb3() {
        ...
        if(provider) {
            console.log('ethereum wallet is connected', provider)
        ...
        const networkData = KriptoBirdz.networks[networkId]
         if(networkData) {
            ...
            for(let i = 0; i < totalSupply; i++) {
                const kBird = await contract.methods.kriptoBirdz(i).call()
                // how should we handle the state on the front end? 
                this.setState({
                    kryptoBirdz:[...this.state.kryptoBirdz, kBird]
                })
            }
            console.log('this', this.state.kryptoBirdz);
